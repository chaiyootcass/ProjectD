...
if (isset($_POST["submit"])) {
    ...
    for ($i = 1; $i <= $num; $i++) {
        ...
        $end = $_POST["st0" . $i];
        mysqli_query($connection, "INSERT INTO station (start_station,end_station) VALUES ('$start','$end')");
    }
    $intermediate_station = $intermediate_station . "," . $_POST["st0d"];
    $intermediate_price = $intermediate_price . "," . $_POST["stf0d"];
    $endstation = $_POST["st0d"];
    $time = date("g:i a", strtotime($time));
    $intermediate_time = $intermediate_time . "," . $time;
    ...
    mysqli_query($connection, "INSERT INTO station (start_station,end_station) VALUES ('$start','$endstation')");
    if ($res) {
